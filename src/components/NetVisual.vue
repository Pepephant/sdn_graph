    ...
    methods:{
        ...
        changeState() {
            ...
            if (now >= 0 && this.state === 0) {
                this.state = 1;
                let edges_green = [1, 3, 12, 13, 27];
                for (let i = 0; i < edges_green.length; i++) {
                    this.$options.network.updateEdge(edges_green[i], {color: 'green'});
                }
                ...
                for (let i = 0; i < edges_red.length; i++) {
                    this.$options.network.updateEdge(edges_red[i], {color: 'red'});
                }
                let edges_orange = [9, 10, 11, 16, 21, 26];
                for (let i = 0; i < edges_orange.length; i++) {
                    this.$options.network.updateEdge(edges_orange[i], {color: 'orange'});
                }
            } else if (now >= 120 && this.state === 2) {
                this.state = 3;
                let edges_red = [1];
                for (let i = 0; i < edges_red.length; i++) {
                    this.$options.network.updateEdge(edges_red[i], {color: 'red'});
